...
describe("LogicHelper", () => {
      ...
      beforeEach(() => {
        Settings.initializeRaw({
          options: {
            [Permalink.OPTIONS.NUM_STARTING_TRIFORCE_SHARDS]: 0,
            [Permalink.OPTIONS.SWORD_MODE]:
              Permalink.SWORD_MODE_OPTIONS.RANDOMIZED_SWORD,
          },
          startingGear: {
            [LogicHelper.ITEMS.PROGRESSIVE_SWORD]: 0,
          },
  ...
  describe("startingItemCount", () => {
    ...
    describe("when the item is not in the starting items", () => {
      test("returns 0", () => {
        const startingItemCount = LogicHelper.startingItemCount(
          "Grappling Hook"
        );
  ...
  describe("isRaceModeDungeon", () => {
    describe("when the dungeon is a race mode dungeon", () => {
      test("returns true", () => {
        const isRaceModeDungeon = LogicHelper.isRaceModeDungeon(
          "Dragon Roost Cavern"
        );
  ...
  describe("caveEntryName", () => {
    test("returns the entry name based on a cave name", () => {
      const entryName = LogicHelper.caveEntryName(
      ...
      beforeEach(() => {
        Locations.locations = {
          "Dragon Roost Cavern": {
            "First Room": {
              need:
                "Grappling Hook & Deku Leaf & DRC Small Key x2 & DRC Big Key",
            },
          },
        };
      });
      ...
      beforeEach(() => {
        Locations.locations = {
          "Dragon Roost Cavern": {
            "Big Key Chest": {
              need:
                "DRC Small Key x1 & Grappling Hook & (DRC Small Key x4 | Deku Leaf | Progressive Bow x2)",
            },
          },
        };
      });
  ...
  describe("_rawRequirementsForLocation", () => {
    describe("when the location has no requirements", () => {
      ...
      test("returns the requirements expression", () => {
        const expression = LogicHelper._rawRequirementsForLocation(
          "Outset Island",
          "Savage Labyrinth - Floor 30"
        );
      ...
      beforeEach(() => {
        Locations.locations = {
          "Outset Island": {
            "Savage Labyrinth - Floor 30": {
              need:
                "Wind Waker | ((Grappling Hook & Hero's Bow) & Hero's Sword) | Skull Hammer",
            },
          },
        };
      });
      ...
      beforeEach(() => {
        Locations.locations = {
          "Outset Island": {
            "Savage Labyrinth - Floor 30": {
              need:
                'Can Access Other Location "Outset Island - Savage Labyrinth - Floor 50"',
            },
            "Savage Labyrinth - Floor 50": {
  ...
  describe("_macroNameForEntrance", () => {
    ...
    test("returns the correct macro name for a cave", () => {
      const macroName = LogicHelper._macroNameForEntrance(
        "Pawprint Isle Wizzrobe Cave"
      );
  ...
  describe("_requirementImplies", () => {
    test("returns true when the second requirement is nothing", () => {
      const implies = LogicHelper._requirementImplies(
        "Grappling Hook",
        "Nothing"
      );
