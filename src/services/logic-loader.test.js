...
describe("LogicLoader", () => {
  beforeEach(() => {
    ...
    global.fetch = (url) => {
      ...
      if (
        url ===
        "https://raw.githubusercontent.com/LagoLunatic/wwrando/1.8.0/logic/item_locations.txt"
      ) {
        mockFileForUrl = itemLocationsFile;
      } else if (
        url ===
        "https://raw.githubusercontent.com/LagoLunatic/wwrando/1.8.0/logic/macros.txt"
      ) {
  ...
  test("returns the item locations file and macros file", async () => {
    ...
    expect(logicFiles).toEqual({
      itemLocationsFile: {
        "Outset Island - Savage Labyrinth - Floor 30": {
          Need:
            "Can Access Savage Labyrinth & Can Defeat Keese & Can Defeat Miniblins",
          "Original item": "Triforce Chart 6",
          Types: "Savage Labyrinth",
